...
import { z } from "zod";
import { db } from "@/db";
import { tours, bookings } from "@/db/schema";
import { ilike, lte, or, eq } from "drizzle-orm";

const openrouter = createOpenRouter({
    apiKey: process.env.OPENROUTER_API_KEY!,
});

const SYSTEM_PROMPT = `You are a friendly and professional customer service assistant for **Golden Rama Tours & Travel**.
...
export async function POST(req: Request) {
    ...
    const result = streamText({
        ...
        tools: chatTools,
        // Allow up to 5 steps: Model text -> Tool call -> Tool result -> Model final text
